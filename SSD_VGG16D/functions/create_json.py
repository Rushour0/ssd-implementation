...
def create_json_data(voc_path, output_path):
    ...
    try:
        ...
        pass
    #Save to file
    with open(output_path + "/TRAIN_images.json", "w") as j:
        json.dump(train_images, j)
    
    with open(output_path + "/TRAIN_objects.json", "w") as j:
    ...
    with open(os.path.join(voc_path, "ImageSets/Main/val.txt")) as f:
        ids = f.read().splitlines()
