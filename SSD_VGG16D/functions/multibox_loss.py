...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, locs_pred, cls_pred, boxes, labels):
        ...
        t_locs = torch.zeros((batch_size, n_default_boxes, 4),
                             dtype=torch.float).to(device)  # (N, 6132, 4)
        t_classes = torch.zeros(
            (batch_size, n_default_boxes), dtype=torch.long).to(device)  # (N, 6132)

        default_boxes_xy = cxcy_to_xy(self.default_boxes)
        for i in range(batch_size):
            n_objects = boxes[i].size(0)

            overlap = find_IoU(boxes[i], default_boxes_xy)  # (n_objects, 6132)
            ...
            object_each_default_box[default_boxes_each_object] = torch.LongTensor(
            ...
            label_each_default_box[overlap_each_default_box <
                                   self.threshold] = 0  # (6132)

            # Save
            t_classes[i] = label_each_default_box

            # Encode pred bboxes
            t_locs[i] = encode_bboxes(xy_to_cxcy(
                boxes[i][object_each_default_box]), self.default_boxes)  # (6132, 4)

        # Identify priors that are positive
        pos_default_boxes = t_classes != 0  # (N, 6132)
        ...
        loc_loss = smooth_L1_loss(
            locs_pred[pos_default_boxes], t_locs[pos_default_boxes])
        ...
        confidence_loss_all = cross_entropy_loss(
            cls_pred.view(-1, num_classes), t_classes.view(-1))  # (N*6132)
        confidence_loss_all = confidence_loss_all.view(
            batch_size, n_default_boxes)  # (N, 6132)

        confidence_pos_loss = confidence_loss_all[pos_default_boxes]

        # Find which priors are hard-negative
        confidence_neg_loss = confidence_loss_all.clone()  # (N, 6132)
        confidence_neg_loss[pos_default_boxes] = 0.
        confidence_neg_loss, _ = confidence_neg_loss.sort(
            dim=1, descending=True)

        hardness_ranks = torch.LongTensor(range(n_default_boxes)).unsqueeze(
            0).expand_as(confidence_neg_loss).to(device)  # (N, 6132)
        ...
        confidence_loss = (confidence_hard_neg_loss.sum() +
